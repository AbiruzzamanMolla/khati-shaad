...
function file_upload(UploadedFile $file, string $path = 'uploads/custom-images/', string $oldFile = '', bool $optimize = false)
{
    ...
    $blockedExtensions = [
        ...
        'sh','bash','zsh','ksh','csh','ps1','py','pl','rb','cgi',
        'exe','msi','dll','so','bin','cmd','bat','com','apk','jar',
        'htaccess','htpasswd','conf','ini',
        'js','ts','html','htm','jsp','asp','aspx','cshtml',
        'rar','7z','tar','gz','bz2'
    ];
...
if (!function_exists('getSettingStatus')) {
    ...
    function getSettingStatus($key, $type = 'string')
    {
        if (Cache::has('setting')) {
            $setting = Cache::memo()->get('setting');
            if (!is_null($key)) {
                if ($type == 'string') {
...
{
    Cache::memo()->rememberForever('CustomPagination', function () {
